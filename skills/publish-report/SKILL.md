...
- To get real image URLs:
  ...
  2. Use the `browser_open` tool to load the page, then `browser_eval` to extract the actual `src` attribute, e.g. `browser_eval("document.querySelector('img[alt*=\"keyword\"]').src")`
